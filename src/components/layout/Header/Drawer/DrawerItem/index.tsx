...
import { useDrawerStore, useDrawerItemStore, useHeaderResetStore } from '@/lib/zustand'
//コンポーネント
...
export default function DrawerItem(
  ...
  const setDrawerChildWidth = useDrawerItemStore((state) => state.setDrawerChildWidth)
    ...
    <>
      {children ? (
        <li className={styles.drawerItem} ref={refDrawerItem}>
          <button data-modifier={drawerChildOpen ? "active" : ""} onClick={drawerChildToggle}>
            {title}
            <FontAwesomeIcon icon={faAngleRight} className={styles.icon}/>
          </button>
          <ul className={styles.drawerChild} data-modifier={drawerChildOpen ? "active" : ""} ref={refDrawerChild}>
            <li>
              <button aria-label="子メニューを閉じる" onClick={drawerChildClose}>
