...
export async function generateStaticParams() {
  ...
  //年月一覧の取得
  const monthlyIndex = groupBy(postsObj.contents)
  //配列に変換
  const monthlyIndexArray = Object.keys(monthlyIndex)

  return monthlyIndexArray.map((index) => ({
    date: index,
  }))
}
...
export default async function Post({params,}:Params) {
  ...
  const postsObj = await getBlogAllByArchive(filters, 1, PER_PAGE)
        ...
        <Layout.Content>
          ...
          <Layout.ContentBox>
            ...
            <Layout.ContentFooter>
              <Function.Pager totalCount={totalCount} PER_PAGE={PER_PAGE} path={pageDatas.pagePath}/>
            </Layout.ContentFooter>
          </Layout.ContentBox>
